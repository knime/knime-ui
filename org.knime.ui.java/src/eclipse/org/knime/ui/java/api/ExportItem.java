...
/**
 * helper class to export files or workflows
 *
 * @author baqueroj
 */
public class ExportItem {
     ...
     *
     * @return true if the legacy workbench dialog is opened
     */
    boolean exportItem(final Space space, final String itemId) {
        final var workbench = PlatformUI.getWorkbench();
        final var url = space.toKnimeUrl(itemId);
        return workbench.getDisplay().syncCall(() -> {
            ...
            final var fileStore = new LocalWorkspaceFileStore(url.getHost(), url.getPath());
            final var item = ContentObject.forFile(fileStore);
            exportWizard.init(PlatformUI.getWorkbench(), new StructuredSelection(item));
