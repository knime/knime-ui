...
/**
 * Testable utility class to create 'ProgressEvents'
 *
 * @author Kai Franze, KNIME GmbH, Germany
 */
final class ProgressEventUtil {
    ...
    static final String PROGRESS_EVENT_NAME = "ProgressEvent";

    /**
     * The status to be sent with a 'ProgressEvent'
     */
    enum Status {
            STARTED("Started"), //
            FETCHING("Fetching"), //
    ...
    private ProgressEventUtil() {
        // Utility class
    }

    /**
     * Creates a {@link IJobChangeListener}.
     */
    static IJobChangeListener createJobChangeListener(final EventConsumer eventConsumer,
        final Predicate<Job> isWatchedJob, final BiConsumer<Job, IProgressMonitor> addProgressListener,
        final BiConsumer<Job, IProgressMonitor> removeProgressListener) {
        return new JobChangeListener(eventConsumer, isWatchedJob, addProgressListener, removeProgressListener);
    }

    /**
     * Creates and consumes a 'ProgressEvent' for the given task, subtask and progress.
     */
    static void createAndSendProgressEvent(final EventConsumer eventConsumer, final String task,
    ...
    private static void createAndSendProgressEvent(final EventConsumer eventConsumer, final String task,
        ...
        eventConsumer.accept(PROGRESS_EVENT_NAME, progressEvent);
    }

    /**
     * Listens to all the jobs the {@link IJobManager} is aware of and sends events via the {@link EventConsumer}
     * whenever a {@link ProfileModificationJob} or {@link LoadMetadataRepositoryJob} starts or finishes.
     *
     * Also, this registers a {@link IProgressMonitor} listener to a job, so we also get job progress information.
     *
     * @author Kai Franze, KNIME GmbH
     * @author Martin Horn, KNIME GmbH
     */
    private static final class JobChangeListener implements IJobChangeListener {
        ...
        JobChangeListener(final EventConsumer eventConsumer, final Predicate<Job> isWatchedJob,
