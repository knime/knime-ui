...
describe('styleRanges util', () => {
    ...
    describe('validation', () => {
        ...
        it('rejects negative range start', () => {
            let result = applyStyleRanges([{ start: -1, length: 1 }, { start: 2, length: 2 }], 'foobar');
            expect(result).toEqual(expected);
        });

        it('rejects too large range end', () => {
            let result = applyStyleRanges([{ start: 0, length: 1 }, { start: 2, length: 20 }], 'foobar');
            expect(result).toEqual(expected);
        });
    ...
    it('applies styles when single range cover text', () => {
        let result = applyStyleRanges([{ start: 0, length: 6, bold: true }], 'foobar');
        let expected = {
            isValid: true,
            textRanges: [
                { bold: true, text: 'foobar' }
            ]
        };
        expect(result).toEqual(expected);
    });
    ...
    it('applies styles when single range does not cover text', () => {
        let result = applyStyleRanges([{ start: 1, length: 3, bold: true }], 'foobar');
        let expected = {
            isValid: true,
            textRanges: [
                { text: 'f' },
