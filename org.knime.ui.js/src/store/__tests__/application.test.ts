...
describe("application store", () => {
  ...
  describe("application Lifecycle", () => {
    ...
    it("destroy application", async () => {
      const { store, dispatchSpy, unsubscribeEventListener } =
        await loadStore();
  ...
  describe("workflow Lifecycle", () => {
    ...
    it("unloads workflow when another one is loaded", async () => {
      const { store, loadWorkflow, unsubscribeEventListener } =
        await loadStore();
      loadWorkflow.mockResolvedValue({
        workflow: { dummy: true, info: { containerId: "root" }, nodes: [] },
        snapshotId: "snap",
  ...
  describe("saved Canvas States", () => {
    ...
    it("saves the canvas state", async () => {
      ...
      expect(
        Object.keys(store.state.application.savedCanvasStates).length
      ).toBe(1);
      expect(
  ...
  describe("workflow preview snapshot", () => {
    ...
    it.todo(
      "should save the current project snapshot when changing to a different project if current is unsaved"
    );
