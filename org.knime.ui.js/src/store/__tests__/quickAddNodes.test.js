...
describe('Quick add node store', () => {
    const createStore = async () => {
        ...
        mockedAPI.noderepository.getNodeRecommendations.mockReturnValue(getNodeRecommendationsResponse);
    ...
    it('creates an empty store', async () => {
        ...
        expect(withoutKeys(store.state.quickAddNodes, nodeSearchStateKeys)).toStrictEqual({
    ...
    describe('actions', () => {
        it('getNodeRecommendations', async () => {
            ...
            let responseWithPorts = withPorts(getNodeRecommendationsResponse, availablePortTypes);
            expect(store.state.quickAddNodes.recommendedNodes).toStrictEqual(responseWithPorts);
        });
    });
});
