...
describe("ConnectorSnappingProvider.vue", () => {
  ...
  const doMount = (customProps = {}) => {
    ...
    ).mockImplementation((...args) => connectNodesMock(...args));
    vi.mocked(
  ...
  describe("placeholder ports", () => {
    ...
    describe("add Port", () => {
      ...
      it.each([
        ...
        async (side, direction, x, connect) => {
          ...
          expect(connectNodesMock).toHaveBeenCalledWith(connect);
        },
      );
    });
    ...
    describe("port type menu", () => {
      ...
      it("handles default port groups correctly on click", async () => {
        ...
        const events = openPortTypeMenuMock.mock.calls[0][0].events;
      ...
      it("sets the preview via menu active items", async () => {
        ...
        expect(setPortTypeMenuPreviewPortMock).toHaveBeenCalledWith(port);
      });
