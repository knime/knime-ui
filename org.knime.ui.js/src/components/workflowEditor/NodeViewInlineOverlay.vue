...
import NodeViewLoader from "@/components/uiExtensions/nodeViews/NodeViewLoader.vue";
...
/**
 * Natural (unscaled) width at which NodeViewLoader renders the view.
 * The view is then CSS-scaled to match the actual on-screen card body size.
 */
const NATURAL_W = 560;

const props = defineProps<{ node: NativeNode }>();
...
const versionId = computed(
  () => workflowStore.activeWorkflow?.info.version ?? CURRENT_STATE_VERSION,
);
...
const screenPos = computed(() => {
  ...
  // and — critically — when the canvas first initializes (containerSize update
  // triggers a zoomFactor/canvasOffset change that would otherwise be missed).
...
<template>
  <div
    class="node-view-inline-overlay"
    :style="{
      left: `${screenPos.x}px`,
      top: `${screenPos.y}px`,
      width: `${screenW}px`,
