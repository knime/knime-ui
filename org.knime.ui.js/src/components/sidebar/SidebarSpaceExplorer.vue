...
export default {
  ...
  computed: {
    ...
    activeProjectOrigin() {
      return this.openProjects.find((p) => p.projectId === this.activeProjectId)
        ?.origin;
    },
  },
  watch: {
  ...
  methods: {
    loadSpaceState() {
      ...
      this.$store.commit(
        ...
        this.activeProjectOrigin.providerId
      );
      this.$store.commit(
        "spaces/setActiveSpaceId",
        this.activeProjectOrigin.spaceId
      );
...
<template>
  <SpaceExplorer mode="mini" @item-changed="onItemChanged" />
</template>
