...
describe("SidebarSpaceExplorer.vue", () => {
  const doMount = ({
          ...
          openProjects: [
            ...
            },
            {
              ...
              origin: {
                ...
                spaceId: "local",
                itemId: "itemX",
                ancestorItemIds: ["parent1", "parent2"],
              },
            },
          ],
        },
    ...
    $store.state.spaces.spaceProviders = {
      someProv1: {
        id: "someProv1",
        spaces: [{ name: "someSpace1", description: "" }],
      },
    };
  ...
  it("remembers state on item change", async () => {
    ...
    wrapper
      ...
      .vm.$emit("item-changed", "changedToId");
    await wrapper.vm.$nextTick();
    expect(dispatchSpy).toHaveBeenCalledWith("spaces/saveLastItemForProject", {
      itemId: "changedToId",
    });
  });
  ...
  it("sets space and provider on mount to the origin of the active project", async () => {
    ...
    expect(commitSpy).toHaveBeenCalledWith(
      "spaces/setActiveSpaceProviderById",
      "someProv1"
    );
    expect(commitSpy).toHaveBeenCalledWith(
    ...
    expect(commitSpy).toHaveBeenCalledWith(
      "spaces/setActiveWorkflowGroupData",
      null
    );
  });
