...
describe("SpaceExplorerActions.vue", () => {
  ...
  describe("normal mode", () => {
    ...
    it("should disable actions", () => {
      ...
      expect(
        wrapper.findComponent(PlusButton).find("button").attributes("disabled")
      ).toBeDefined();
    });
    ...
    it.each([
      ...
      'should emit an "action:%s" event when clicking on the relevant action',
      (actionId) => {
        const { wrapper } = doMount();
    ...
    it.each([
      ...
      (actionId) => {
        ...
        wrapper
          ...
          .vm.$emit("item-click", null, { id: actionId });
