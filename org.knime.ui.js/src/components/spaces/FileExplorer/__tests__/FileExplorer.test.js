...
describe("FileExplorer.vue", () => {
  ...
  describe("selection", () => {
    it("should select items and emit selected ones", async () => {
      ...
      expect(wrapper.findAll(".file-explorer-item").at(1).classes()).toContain(
        "selected"
      );
      expect(wrapper.findAll(".file-explorer-item").at(2).classes()).toContain(
        "selected"
      );
      expect(wrapper.findAll(".file-explorer-item").at(3).classes()).toContain(
        "selected"
      );
      expect(wrapper.findAll(".file-explorer-item").at(5).classes()).toContain(
        "selected"
      );
  ...
  describe("options menu", () => {
    ...
    const getOptionsItem = (optionsMenu, index) =>
      optionsMenu.findAll(".clickable-item").at(index);
    ...
    it("should show the item option button", () => {
      const { wrapper } = doMount();
      expect(wrapper.find(".submenu").find(".submenu-toggle").exists()).toBe(
        true
      );
    });
    ...
    it("should have the delete option", async () => {
      ...
      expect(
        menuItem.findAll("li").at(DELETE_OPTION_IDX).element.innerHTML
      ).toContain("Delete");
    });
    ...
    it("should enable the delete option for items that specify it", async () => {
      ...
      const secondFileItem = wrapper
        .findAll(".file-explorer-item")
        .at(indexWithDeletable);

      await openSubMenu(wrapper, 0);

      const notDeletableMenuItem = firstFileItem
        .find(".menu-wrapper")
        .findAll("li")
        .at(1);
      expect(notDeletableMenuItem.find(".disabled").exists()).toBe(true);
      expect(notDeletableMenuItem.element.title).toBe(
        "Open workflows cannot be deleted"
      );

      // open submenu
      await openSubMenu(wrapper, indexWithDeletable);

      const deletableMenuItem = secondFileItem
        .find(".menu-wrapper")
        .findAll("li")
        .at(1);
    ...
    it("should emit deleteItems on delete option click", async () => {
      ...
      const { wrapper } = doMount({
        props: {
          items: MOCK_DATA.map((item) => ({ ...item, canBeDeleted: true })),
        },
      });
      ...
      expect(wrapper.emitted("deleteItems")[0][0]).toMatchObject({
        items: [{ id: `${itemIdx}` }],
      });
    });
    ...
    it("should have rename enabled when item is not open", async () => {
      ...
      const menuItem = wrapper.find(".submenu").find(".menu-wrapper");
      expect(menuItem.findAll("li").at(0).element.innerHTML).toContain(
        "Rename"
      );
    });
    ...
    it("should have rename disabled when item is open", async () => {
      ...
      const menuItem = wrapper.find(".submenu").find(".menu-wrapper");
      expect(menuItem.findAll("li").at(0).element.outerHTML).toContain(
    ...
    it("should automatically trim new name", async () => {
      ...
      expect(wrapper.emitted("renameFile")).toBeTruthy();
      expect(wrapper.emitted("renameFile")[0][0].newName).toBe(
        "New Folder name"
      );
    });
