...
const runSchemaValidation = async (
  ...
  const textDocument = TextDocument.create(
    "http://org.knime.ui/layout-editor-schema.json",
    "json",
    model.getVersionId(),
    content,
  );
...
const getSchema = async (): Promise<SchemaConfig> => {
  ...
  // schema.$id = `inmemory://schemas/layout-editor-view-layout-${Date.now()}.json`;
  ...
  const schemaConfig = {
    ...
    uri: `http://org.knime.ui/layout-editor-schema-${Date.now()}.json`,
    fileMatch: ["*.json"],
    schema,
  };
