...
describe('MoveableNodeContainer', () => {
    ...
    const doMount = ({
        ...
        return { wrapper, $store, actions, mutations };
    };
    ...
    describe('moving', () => {
        ...
        it('does not deselect nodes when node is already selected', () => {
            const { actions } = doMount({ props: { id: 'root:2' }, isNodeSelected: jest.fn(() => true) });
    ...
    it('should abort moving a node when Esc is pressed', () => {
        const { wrapper, mutations, $store } = doMount({
            isDragging: true
        });
