...
describe('ColorSelectionDialog.vue', () => {
    ...
    it('should emit an event when a color is hovered', async () => {
        ...
        await wrapper.findAllComponents(FunctionButton).at(1).find('button').trigger('mouseenter', { stopPropagation });
        expect(wrapper.emitted('hoverColor')[0][0]).toBe(annotationColorPresets.Avocado);
        expect(stopPropagation).toHaveBeenCalled();

        await wrapper.findAllComponents(FunctionButton).at(1).find('button').trigger('mouseleave', { stopPropagation });
    ...
    it('should emit an event when a color is selected', async () => {
        ...
        const stopPropagation = vi.fn();
        await wrapper.findAllComponents(FunctionButton).at(1).find('button').trigger('click', { stopPropagation });
        expect(wrapper.emitted('selectColor')[0][0]).toBe(annotationColorPresets.Avocado);
        expect(stopPropagation).toHaveBeenCalled();

        // also handles 'none' (aka: 'white') option
        await wrapper.findAllComponents(FunctionButton).at(0).find('button').trigger('click', { stopPropagation });
        expect(wrapper.emitted('selectColor')[1][0]).toBe(annotationColorPresets.None);
    });
    ...
    it('should handle the white color', async () => {
        ...
        expect(wrapper.findAllComponents(FunctionButton).at(0).props('active')).toBe(false);
        ...
        const colorButton = wrapper.findAllComponents(FunctionButton).at(0).find('button');
