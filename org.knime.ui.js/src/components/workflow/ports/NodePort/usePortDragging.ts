...
const createConnectorFromEvent = (params: Params, absolutePoint: [number, number]): DragConnector => {
...
export const usePortDragging = (params: Params) => {
    ...
    useEscapeStack({
        ...
        onEscape: () => {
            ...
            if (dragConnector.value) {
                const { removeConnector } = params.onEscPressed?.() || { removeConnector: true };
    ...
    const onPointerMove = throttle((event: PointerEvent) => {
        ...
        const [absoluteX, absoluteY] = screenToCanvasCoordinates.value([event.x, event.y]);
    ...
    const onLostPointerCapture = () => {
        ...
        if (lastHitTarget && lastHitTarget.allowsDrop) {
            lastHitTarget.element.dispatchEvent(new CustomEvent('connector-leave', { bubbles: true }));
        }
