...
export type TrackRef<T, K extends keyof OmitUndef<T>> = T extends undefined
  ? ToRef<OmitUndef<T>[K] | undefined>
  : ToRef<OmitUndef<T>[K]>;

export function useTrack<T, K extends keyof OmitUndef<T>>(
...
export function useTrack<T, K extends keyof OmitUndef<T>>(
  ...
  const trackRef = customRef((track, trigger) => ({
    ...
    set: (value: any) => {
      ...
      vtg ? (vtg[key as keyof T] = value) : (defaultValue = value);
      trigger();
    },
  }));
  whenever(
