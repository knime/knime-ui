...
export type Hook<T = () => void> = T | TransitionVars | TransitionVars[];
export interface EasingFunction {
  (p: number): number;
}

export interface PixiMatrix {
  a: number;
  b: number;
  c: number;
  d: number;
...
export interface TransitionVars extends PixiVars {
  delay?: number;
  duration?: number;
  ease?: CubicBezierPoints | EasingFunction;
}

export interface TransitionTicker {
  duration: number;
