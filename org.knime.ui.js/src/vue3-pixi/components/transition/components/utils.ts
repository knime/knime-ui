...
export async function callInstanceSetterHook(
  instance: any,
  props: any,
  name: string,
) {
  ...
  filters.forEach((filter: any) =>
    callInstanceSetterHook(filter, filter, name),
  );
  setProps(instance, toArray(hook));
}

export async function callAnimationHook(
  instance: any,
  props: any,
  name: string,
  done: Fn,
...
async function executeTransition(
  ...
  const fields = Object.keys(transition).filter(
    (i) => !optionsKeys.includes(i),
  );
...
async function executeInTicker(
  ticker: TransitionTicker | void,
  done: Fn,
  abort: Fn,
...
async function executeOutTicker(
  ticker: TransitionTicker | void,
  done: Fn,
  abort: Fn,
  context: TransitionOptions["context"],
