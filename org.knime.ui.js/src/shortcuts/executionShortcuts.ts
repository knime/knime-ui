...
export type ExecutionShortcuts = UnionToShortcutRegistry<
  | "executeAll"
  | "cancelAll"
  | "resetAll"
  | "executeSelected"
  ...
  ...conditionGroup(
    ({ $store }) => $store.state.application.permissions.canEditWorkflow,
    {
      executeAll: {
        ...
        icon: ExecuteAllIcon,
        execute: ({ $store }) =>
          $store.dispatch("workflow/executeNodes", "all"),
        condition: ({ $store }) =>
          Boolean(
      ...
      resumeLoopExecution: {
        ...
        condition: ({ $store }) =>
          $store.getters["selection/singleSelectedNode"]?.loopInfo
