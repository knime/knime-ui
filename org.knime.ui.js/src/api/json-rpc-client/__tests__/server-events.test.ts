...
import { registerNotificationHandler, serverEventHandler } from '../server-events';
...
describe('server-events', () => {
    ...
    it('fails and does not call composite event if one event does not exist', () => {
        const rawServerEvent = '{"eventType":"WorkingEvent:NotWorkingEvent","payload":{"events":[null,null]}}';
        expect(() => {
    ...
    describe('error handling', () => {
        ...
        it('returns an error for non-existing methods', () => {
            expect(() => {
                serverEventHandler('{"jsonrpc":"2.0","eventType":"invalidAction","params":["foo"]}');
            }).toThrow();
        });

        it('returns an error for non-function handlers', () => {
            expect(() => {
                serverEventHandler('{"jsonrpc":"2.0","eventType":"NotFunction","params":["foo"]}');
            }).toThrow();
        });
    });
});
