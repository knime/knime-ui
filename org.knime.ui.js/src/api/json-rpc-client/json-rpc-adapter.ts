  ...
  if (typeof response === "string") {
    try {
      ...
      throw createWrappedError(
        `Could not be parsed as JSON-RPC: ${response}`,
        error
      );
    }
  }
...
const validateResponse = ({
  ...
  if (result) {
    consola.error(
      `Invalid JSON-RPC response: Both error and result exist.\n${JSON.stringify(
        maybeResponse
      )}`
    );

    throw createWrappedError(
      `Error returned from JSON-RPC API ${JSON.stringify([
        method,
        params,
...
const handleResponse = ({
  ...
  response: string | JSONRPCResponse;
  method: string;
  params: JSONRPCParams;
}) => {
  try {
